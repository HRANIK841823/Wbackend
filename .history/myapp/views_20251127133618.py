...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        # 2. Check if the value is a valid ObjectId format
        if not ObjectId.is_valid(lookup_value):
            # If the provided string is not a valid ObjectId format, raise 404
            raise NotFound(detail="Not found.")

        # 3. Convert the string to a MongoDB ObjectId instance
        object_id = ObjectId(lookup_value)

        # 4. Use the custom object_id to filter the queryset
        try:
            # We use the filter method and access the first item
            obj = self.get_queryset().get(_id=object_id)
            return obj
        except Product.DoesNotExist:
            # If the object is not found, raise 404
            raise NotFound(detail="Not found.")
