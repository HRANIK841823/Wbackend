...
#     # 1. Get the user's ID (which is the MongoDB ObjectId)
#     user_id = request.user.id 
    
#     # Check if user_id is None (shouldn't happen with IsAuthenticated, but safe to check)
#     if not user_id:
#         return Response({'error': 'User ID is missing for filtering.'}, status=status.HTTP_400_BAD_REQUEST)

#     try:
#         # 💡 THE MOST RELIABLE FIX: Filter using the explicit ID field, 'buyer_id'.
#         # This tells the ORM exactly which field (which stores the ObjectId) to query.
#         products = Product.objects.filter(buyer_id=user_id).order_by('-created_at') 

#         serializer = ProductSerializer(products, many=True)
        
#         # ⚠️ CRITICAL DEBUG STEP: Remove or minimize print statements if they are causing issues
#         # For a 500 error, sometimes complex logging can be the culprit if it hits a None object.
#         # It's safest to remove the loop for now.
        
#         return Response(serializer.data)
        
#     except Exception as e:
#         # If the 500 error persists, this block will catch it and give you the real error name.
#         print(f"!!! MY_PURCHASE_HISTORY CRASH !!!: {type(e).__name__}: {e}")
#         return Response(
#             {'error': f'A transaction error occurred: {type(e).__name__}', 'detail': str(e)}, 
#             status=status.HTTP_500_INTERNAL_SERVER_ERROR
#         )
...
def list_all_users(request):
    """
    Retrieves and serializes a list of all user profiles.
    """
    try:
        # 1. Fetch all user objects from the database
        users = User.objects.all() 
        
        # 2. Serialize the queryset: must use 'many=True' for a list
        serializer = UserProfileSerializer(users, many=True) 
        
        # 3. Return the list of serialized user data
        return Response(serializer.data, status=status.HTTP_200_OK)
    
    except Exception as e:
        print(f"!!! LIST_ALL_USERS CRASH !!!: {type(e).__name__}: {e}")
        return Response(
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    # 💡 FIX 1: Allow listing and detail viewing without authentication
    def get_permissions(self):
        # Allow anyone to view the list and retrieve details
        if self.action in ['list', 'retrieve']:
            return [permissions.AllowAny()]
        # Require authentication for all other actions (create, update, delete, custom actions)
        return [permissions.IsAuthenticated()]

    def get_object(self):
        # Handles MongoDB ObjectId lookup for detail view, as provided previously
        lookup_value = self.kwargs[self.lookup_field] 
        if not ObjectId.is_valid(lookup_value):
            raise NotFound(detail="Not found.")
        object_id = ObjectId(lookup_value)
    ...
    def get_queryset(self):
        ...
        # By default, for the main list, we only show 'available' items
        if self.action == 'list':
            # Note: Ensure your test data has status='available' to appear here
            return queryset.filter(status='available')
    ...
    # --- CUSTOM ACTIONS (Buy is left here because it's detail-specific) ---

    @action(detail=True, methods=['post'])
    def buy(self, request, id=None): 
        """
        Logic to buy an item: Requires user to be logged in (handled by get_permissions).
        """
        try:
            product = self.get_object() 
        except NotFound:
...
# --- STANDALONE HISTORY VIEWS ---

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def my_purchase_history(request):
    """
    Shows items the current authenticated user has bought.
    """
    user_id = request.user.id 
    ...
    try:
        # Filter by the user's ID stored in the buyer_id field
        products = Product.objects.filter(buyer_id=user_id).order_by('-created_at') 
        serializer = ProductSerializer(products, many=True)
        return Response(serializer.data)
...
# 💡 NEW STANDALONE VIEW FOR SELL HISTORY (Moved from ViewSet)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def my_sell_history(request):
    """
    Shows all items (Available or Sold) posted by the current authenticated user.
    """
    user_id = request.user.id
    ...
    try:
        # Filter by the user's ID stored in the seller field
        products = Product.objects.filter(seller=user_id).order_by('-created_at')
        serializer = ProductSerializer(products, many=True)
        return Response(serializer.data)
    except Exception as e:
        print(f"!!! MY_SELL_HISTORY CRASH !!!: {type(e).__name__}: {e}")
        return Response(
            {'error': f'A server error occurred: {type(e).__name__}', 'detail': str(e)}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )