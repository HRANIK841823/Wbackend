...
def my_purchase_history(request):
    # Convert user.id → MongoDB ObjectId
    user_object_id = ObjectId(str(request.user.id))

    # Filter products where buyer_id matches ObjectId
    products = Product.objects.filter(buyer_id=user_object_id).order_by('-created_at')
