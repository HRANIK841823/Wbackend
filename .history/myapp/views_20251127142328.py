...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
    # ... (code for lookup_value, ObjectId.is_valid, and conversion to object_id) ...

    try:
        # FIX: Ensure you are using .get() for efficiency, not .filter().get()
        # Your original code was: 
        # obj = self.get_queryset().get(_id=object_id)
        # This is technically fine, but using .get() directly on the base manager is clearer
        # when you already know the primary key.
        obj = Product.objects.get(_id=ObjectId) # Use Product.objects.get
        return obj
