...
def my_purchase_history(request):
    # 1. Get the raw ID (which should be an ObjectId instance or string)
    user_id_raw = request.user.id 
    
    # 2. 💡 CRITICAL FIX: Ensure the ID is a valid ObjectId instance
    try:
        # Convert it to ObjectId instance if it's a string, or just use it.
        # This explicitly tells the ORM what type of value it's searching for.
        if not isinstance(user_id_raw, ObjectId):
            # Check for valid format before conversion
            if not ObjectId.is_valid(user_id_raw):
                 # This should never happen if the ID is from request.user
        ...
        # Handle the case where the user_id itself is malformed
        return Response({'error': f'Invalid User ID format: {e}'}, status=status.HTTP_400_BAD_REQUEST)

    # 3. Use the explicitly-typed ObjectId for filtering
    # Use the buyer__in=[user_id] pattern which is sometimes more robust
    products = Product.objects.filter(buyer__in=[user_id]).order_by('-created_at')

    # ... (Continue with serialization)
    serializer = ProductSerializer(products, many=True)
    
    # Debug info: show buyer_id and user_id in logs (keep this!)
    for product in products:
        # Access buyer_id directly as a string for robust logging
        print(f"API CALL: Product: {product.title}, buyer_id: {product.buyer_id}, user_id: {user_id}")
