...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        # 1. Get the lookup value (this will be the integer product_number)
        lookup_value = self.kwargs[self.lookup_field] 
        
        # We expect an integer here, so we cast the URL string to int
        try:
            lookup_number = int(lookup_value)
        except ValueError:
            raise NotFound(detail="Invalid product number format.")

        try:
            # 🎯 FIX: Query using the product_number field 🎯
            obj = Product.objects.get(product_number=lookup_number) 
            return obj
        
        except Product.DoesNotExist:
            raise NotFound(detail="Product not found.")
        except Exception as e:
            # Use the robust exception handling
            print(f"Error during product lookup: {e}") 
            raise NotFound(detail="Product not found.")
