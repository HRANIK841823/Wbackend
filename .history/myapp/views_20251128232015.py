...
def my_purchase_history(request):
    user_id = request.user.id # Get the user's ObjectId
    
    # 💡 FIX: Filter by the user's ID (ObjectId) for reliable lookup
    products = Product.objects.filter(buyer=user_id).order_by('-created_at') 

    serializer = ProductSerializer(products, many=True)
    
    # Debug info: show buyer_id and user_id in logs (keep this for testing)
