...
def my_purchase_history(request):
    ...
    products = Product.objects.filter(buyer=user).order_by('-created_at')  # Use object directly

    serializer = ProductSerializer(products, many=True)
    
    # Debug info: show buyer_id and user_id in logs
    for product in products:
        print(f"API CALL: Product: {product.title}, buyer_id: {product.buyer.id if product.buyer else None}, user_id: {request.user.id}")
